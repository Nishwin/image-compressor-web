...
def compress_folder(
    ...
    res_label = f"{target_size[0]}×{target_size[1]}" if target_size else "original"
    log_lines.append(f"Starting recursive image conversion to {fmt.upper()}... {'(DRY RUN)' if dry_run else ''}")
    log_lines.append(f"Found {total_files} image(s) to process. Target resolution: {res_label}.")
