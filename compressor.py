...
def compress_folder(
    ...
    log_lines.append(f"Starting recursive image conversion... {'(DRY RUN)' if dry_run else ''}")
                        ...
                        if not dry_run:
                            if has_alpha:
                                ...
                                img.save(
                                    output_path,
                                    "webp",
                                    lossless=use_lossless_for_pngs,
                                    quality=png_quality if not use_lossless_for_pngs else 100,
                                )
                            else:
                                img = img.convert("RGB")
                                quality = jpg_quality if ext in ('.jpg', '.jpeg') else default_quality
