...
def parse_target_size(resolution_str):
    """Convert '1920x1080' → (1920, 1080), 'none'/'' → None."""
    if not resolution_str or resolution_str.lower() == "none":
        return None
    try:
...
def worker_process(job_id, input_zip_path, work_dir, output_format="webp", strength="medium", compression_type="lossy", target_size=None):
    """
...
def worker_process_folder(job_id, input_folder, work_dir, output_format="webp", strength="medium", compression_type="lossy", target_size=None):
    """
...
@app.route("/image-compressor", methods=["GET"])
...
def upload_async():
    ...
    target_size = parse_target_size(request.form.get("target_resolution", "none"))
    ...
    with jobs_lock:
        jobs[job_id] = {
            ...
            "log": f"Job created at {time.ctime()}\nJob folder: {work_dir}\nOutput format: {output_format} | Strength: {strength} | Type: {compression_type} | Resolution: {target_size or 'original'}\n",
            "stats": None,
            "progress": None,
    ...
    t = threading.Thread(target=worker_process, args=(job_id, input_zip_path, work_dir, output_format, strength, compression_type, target_size), daemon=True)
    t.start()
...
def upload_folder_async():
    ...
    t = threading.Thread(target=worker_process_folder, args=(job_id, input_folder, work_dir, output_format, strength, compression_type, target_size), daemon=True)
    t.start()
